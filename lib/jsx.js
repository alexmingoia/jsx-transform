 ...
 * @param {Object=} options.renameAttrs rename attributes while desugaring JSX (i.e. `class` to `className`).
 ...
 * @param {Boolean=false} options.docblockUnkownTags Handle unknown tags like known tags,
 * except passing them as an object.
...
function visitNode(traverse, object, path, state) {
  ...
  } else if (options.docblockUnkownTags) {
  ...
  if (!knownTag && options.docblockUnkownTags) {
    utils.append(', ', state);
  } else {

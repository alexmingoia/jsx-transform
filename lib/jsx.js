...
function parse(str, options) {
  options = (options || { constructor: 'virtualdom.h' });
  return transform([visitNode], str, options).code;
};
...
function parseFile(path, options) {
  return parse(fs.readFileSync(path, 'utf8'), options);
};

// Visit JSX tag and transform into virtual DOM function call.
function visitNode(traverse, object, path, state) {
  var ident = (utils.getDocblock(state).jsx || state.g.opts.constructor);
  ...
  var identTag = (nameObj.type === Syntax.XJSIdentifier);
  utils.append(identTag ? ident + '("': '', state);
...
visitNode.test = function(object, path, state) {
  var jsx = (utils.getDocblock(state).jsx || state.g.opts.constructor);
